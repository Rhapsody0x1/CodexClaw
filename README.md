...
- `/help` / `/帮助`：查看命令列表；
- `/status` / `/状态`：查看当前会话状态；
- `/new [工作目录]` / `/新建 [工作目录]`：将前台（若有内容）转后台并新建临时前台；可选指定工作目录，支持绝对路径，相对路径默认按当前前台工作目录解析；
- `/stop` / `/停止`：结束当前前台会话（已保存则保留，未保存则丢弃）并新建临时前台；
- `/interrupt` / `/中断`：仅停止当前运行，不结束会话；
- `/lang [en|zh|status]` / `/语言 [en|zh|status]`：切换界面语言；
- `/model [name|inherit|status]` / `/模型 [name|inherit|status]`：设置或查看当前模型；
- `/fast [on|off|inherit|status]` / `/快速 [on|off|inherit|status]`：设置 Fast 模式；
- `/context [1m|standard|inherit|status]` / `/上下文 [1m|standard|inherit|status]`：设置上下文模式；
- `/reasoning [low|medium|high|xhigh|inherit|status]` / `/思考 [low|medium|high|xhigh|inherit|status]`：设置思考深度；
- `/verbose [on|off|status]` / `/详细 [on|off|status]`：切换工具输出的简略或详细模式；
- `/sessions [all]` / `/会话 [all]`：先查看项目（按目录）列表；
- `/sessions <项目编号> [page]` / `/会话 <项目编号> [page]`：查看该项目下的会话列表；
- `/import` / `/导入`：查看系统 `~/.codex/sessions` 中可导入的项目；
- `/import <项目编号> [page]` / `/导入 <项目编号> [page]`：查看该项目下可导入会话；
- `/import <编号|会话ID>` / `/导入 <编号|会话ID>`：导入系统会话到 `~/.codex-claw/.codex/sessions`；
- `/resume <编号|会话ID>` / `/恢复 <编号|会话ID>`：把磁盘会话恢复到前台；
- `/loadbg <编号|会话ID> [alias]` / `/载入后台 <编号|会话ID> [alias]`：把磁盘会话加载到后台；
- `/bg [alias]` / `/后台 [alias]`：把当前前台会话转入后台；
- `/fg <alias>` / `/前台 <alias>`：把后台会话切回前台；
- `/rename <old_alias> <new_alias>` / `/重命名 <old_alias> <new_alias>`：重命名后台会话标签；
- `/save` / `/保存`：显式保存当前前台会话；
- `/compact` / `/压缩`：手动压缩当前会话上下文；
- `/alias` / `/别名`：管理命令别名；
- `/self-update` / `/自更新`：构建当前仓库、替换运行中的二进制并退出当前进程；若已配置外部守护服务，则可自动拉起新版本。
