...
- 跨模块或 HTTP 流程的集成测试放在 `tests/integration_flow.rs`。
...
CodexClaw is a personal Rust project by **Rhapsody0x1** — a QQ chat bot powered by OpenAI Codex. Bug reports and discussions are welcome; feature requests are considered at the author's discretion. You are free to fork the project for your own needs.

### Quick Reference

- **Build:** `cargo check` / `cargo build` / `cargo run`
- **Test:** `cargo test`
...
Follow `rustfmt` defaults with 4-space indentation. Use `snake_case` for functions and modules, `PascalCase` for types. Return `anyhow::Result` at application boundaries. Keep async boundaries explicit.

### Commits

Use conventional commit prefixes: `feat`, `fix`, `refactor`, `doc`. Write short imperative subjects scoped to one logical change (e.g., `feat(qq): add group message handler`).

### Internationalization

Locale files live in `locales/en.yml` and `locales/zh.yml`. Use the `rust_i18n::t!` macro for translated strings. Every new command must have both English and Chinese aliases.

### Security

Never commit real credentials. Load secrets from a local TOML file via the `CODEX_CLAW_CONFIG` environment variable. Treat `data/` as ephemeral runtime state.
