...
| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `alias` | 字符串 | 否 | 后台会话的别名标签；省略时自动生成 |

**行为：**
- 将当前前台会话存入后台；可指定别名，省略时自动生成，并在后续 `/fg` ↔ `/bg` 切换中保持该名称。
- 若空白临时前台的首轮对话仍在运行，先保留指定或自动生成的别名，待该轮建立 thread 后再存入后台。
- 若前台是空白临时会话且没有运行中的轮次，不转入后台，仅重置为新的临时会话。
...
- 无参数：直接切回最近存入后台的会话。若无后台会话，提示后台为空。
...
以下命令在无参数时会进入交互式设置模式：`/model`、`/fast`、`/context`、`/reasoning`、`/verbose`、`/lang`、`/approvals`、`/plan`、`/sessions`、`/import`、`/resume`、`/loadbg`。
